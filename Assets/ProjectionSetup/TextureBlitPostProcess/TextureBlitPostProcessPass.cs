...
[System.Serializable]
public class TextureBlitPostProcessPass : ScriptableRenderPass
{
	...
	public TextureBlitPostProcessPass() {
	...
	public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData) {
		...
		Material postProcessMaterial = new Material(Shader.Find("Hidden/Shader/TextureBlitPostProcess"));
		if (postProcessMaterial == null) {
			Debug.LogError("TextureBlitPostProcess shader was not found.");
			return;
		}

		CommandBuffer cmd = CommandBufferPool.Get("TextureBlitPostProcessRenderer");
		cmd.Clear();
		...
		//---Custom effect here---
		var customEffect = stack.GetComponent<TextureBlitPostProcess>();
		// Only process if the effect is active
		if (customEffect.IsActive()) {
			// P.s. optimize by caching the property ID somewhere else
			postProcessMaterial.SetVector(Shader.PropertyToID("_InputResolution"), customEffect.inputResolution.value);
			postProcessMaterial.SetTexture(Shader.PropertyToID("_TextureToBlit1"), customEffect.textureToBlit1.value);
			postProcessMaterial.SetTexture(Shader.PropertyToID("_TextureToBlit2"), customEffect.textureToBlit2.value);
			postProcessMaterial.SetTexture(Shader.PropertyToID("_TextureToBlit3"), customEffect.textureToBlit3.value);
			postProcessMaterial.SetTexture(Shader.PropertyToID("_TextureToBlit4"), customEffect.textureToBlit4.value);

			BlitTo(postProcessMaterial);
		}
